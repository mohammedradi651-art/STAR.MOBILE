...
export default function ElectricityPage() {
  ...
  const handleSearch = async () => {
    ...
    setResult(null);
    try {
      const response = await fetch('/api/electricity/lookup', {
        method: 'POST',
  ...
  const handlePayNow = async () => {
    ...
    try {
        ...
        batch.set(txRef, {
            ...
            transactionType: 'سداد فاتورة كهرباء',
            notes: `رقم المشترك: ${result.subscriberNumber} - الاسم: ${result.name}`,
            recipientPhoneNumber: result.subscriberNumber
        });

        const requestRef = doc(firestoreCollection(firestore, 'electricityRequests'));
          ...
          <Card className="w-full max-sm text-center shadow-2xl rounded-[40px] overflow-hidden border-none bg-card">
            ...
            <CardContent className="p-8 space-y-6">
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">المؤسسة العامة للكهرباء - نظام السداد المباشر</p>
              </div>
  ...
  return (
    <div className="flex flex-col h-full bg-[#F4F7F9] dark:bg-slate-950">
      <SimpleHeader title="سداد الكهرباء" />

      {(isSearching || isProcessing) && <ElectricityMovingLoader />}
            ...
            <div className="relative flex flex-col items-center text-center space-y-4">
                <div className="bg-white p-2 rounded-[24px] shadow-2xl animate-in zoom-in-95 duration-700 overflow-hidden border-2 border-white">
                    <div className="relative h-16 w-24">
                        <Image src="https://i.postimg.cc/3RbLf0J5/images-(6).jpg" alt="الكهرباء" fill className="object-contain" priority />
                    </div>
                </div>
                <div className="space-y-1">
                    <h2 className="text-xl font-black text-white tracking-tight">سداد فواتير الكهرباء</h2>
                            ...
                            <div className="bg-white dark:bg-slate-900 rounded-[28px] p-6 shadow-xl border border-slate-100 dark:border-slate-800" dir="rtl">
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    <div className="bg-slate-50 dark:bg-slate-800 p-4 rounded-2xl text-center border border-slate-100 dark:border-slate-700">
                                        <p className="text-[9px] font-bold text-muted-foreground uppercase mb-1">تاريخ الفاتورة</p>
                                        <p className="text-xs font-black">{result.date}</p>
                                    </div>
                                    <div className="bg-slate-50 dark:bg-slate-800 p-4 rounded-2xl text-center border border-slate-100 dark:border-slate-700">
