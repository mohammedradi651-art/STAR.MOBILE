...
import { User, Phone, Check, X, Archive, Inbox, Zap, Hash, Calendar, Wallet, Trash2, MessageCircle } from 'lucide-react';
...
const StatusBadge = ({ status }: { status: ElectricityRequest['status'] }) => {
  const statusStyles = {
...
export default function ElectricityRequestsPage() {
  const firestore = useFirestore();
  const { toast } = useToast();
  const [selectedRequest, setSelectedRequest] = useState<ElectricityRequest | null>(null);
  ...
  const handleComplete = async (requestId: string) => {
    if (!firestore) return;
    try {
      await updateDoc(doc(firestore, 'electricityRequests', requestId), { status: 'completed' });
  ...
  const handleCancelAndRefund = async () => {
    ...
    const reqRef = doc(firestore, 'electricityRequests', selectedRequest.id);
    ...
    try {
      ...
      batch.set(txRef, {
        ...
        notes: `إلغاء طلب رقم: ${selectedRequest.subscriberNumber}. ${cancelNote ? `السبب: ${cancelNote}` : ''}`
      });

      batch.set(notifRef, {
        title: 'إلغاء طلب سداد كهرباء',
        body: `نعتذر، تم إلغاء طلب سداد فاتورة الكهرباء للرقم ${selectedRequest.subscriberNumber} وإرجاع المبلغ لحسابك.`,
  ...
  return (
    <div className="flex flex-col h-full bg-background">
      <SimpleHeader title="طلبات الكهرباء" />
        ...
        <div className="flex-1 overflow-y-auto p-4 no-scrollbar">
          <TabsContent value="active" className="mt-0 space-y-3">
            {isLoading ? (
                ...
                <p className="text-center text-muted-foreground py-20">لا توجد طلبات معلقة حالياً.</p>
            ) : (
                    ...
                    <Card key={req.id} className="cursor-pointer hover:bg-muted/30 transition-all rounded-[28px] border-none shadow-sm" onClick={() => { setSelectedRequest(req); setIsDialogOpen(true); }}>
                        <CardContent className="p-4 flex justify-between items-center">
                            <div className="flex items-center gap-3">
                                <div className="p-2.5 bg-orange-500/10 rounded-xl"><Zap className="h-5 w-5 text-orange-600" /></div>
                                <div className="text-right">
                                    <p className="font-black text-sm">{req.subscriberName}</p>
                                    <p className="text-[10px] text-muted-foreground font-bold">الرقم: {req.subscriberNumber}</p>
                                </div>
                            </div>
                            <div className="text-left">
                                <p className="font-black text-orange-600 text-sm">{req.totalAmount.toLocaleString()} ر.ي</p>
                ...
                <Card key={req.id} className="opacity-70 rounded-[28px] border-none shadow-sm" onClick={() => { setSelectedRequest(req); setIsDialogOpen(true); }}>
                    <CardContent className="p-4 flex justify-between items-center">
                        <div className="flex items-center gap-3">
                            ...
                            <div className="text-right">
                                <p className="font-black text-sm">{req.subscriberName}</p>
                                <p className="text-[10px] text-muted-foreground">الرقم: {req.subscriberNumber}</p>
                            </div>
                        </div>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-w-sm rounded-[32px] overflow-hidden p-0 border-none shadow-2xl">
            <div className="bg-orange-600 p-6 text-center text-white">
                <DialogHeader>
                    <DialogTitle className="text-white text-center font-black">تفاصيل السداد</DialogTitle>
                </DialogHeader>
            </div>
          {selectedRequest && (
            <div className="p-6 space-y-4">
              <div className="space-y-1">
                ...
                <InfoRow icon={Zap} label="العمولة" value={`${selectedRequest.commission} ر.ي`} />
                <div className="flex justify-between items-center py-3 bg-muted/50 rounded-xl px-2 mt-2">
                    ...
                    <span className="font-black text-orange-600 text-base">{selectedRequest.totalAmount.toLocaleString()} ر.ي</span>
                </div>
                <hr className="my-2 border-dashed" />
              ...
              {selectedRequest.status === 'pending' && (
                <div className="grid grid-cols-2 gap-3 pt-4">
                    ...
                    <Button className="rounded-2xl h-12 font-black bg-orange-600 hover:bg-orange-700" onClick={() => handleComplete(selectedRequest.id)}>
      ...
      <AlertDialog open={isCancelAlertOpen} onOpenChange={setIsCancelAlertOpen}>
        <AlertDialogContent className="rounded-[32px]">
          ...
          <div className="py-2 space-y-2">
            ...
            <Textarea placeholder="اكتب السبب هنا ليعرف المشترك..." value={cancelNote} onChange={(e) => setCancelNote(e.target.value)} className="rounded-2xl bg-muted/30 border-none" />
          </div>
